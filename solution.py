def solution(A, D):
    ...
    for index, transaction in enumerate(A):
        ...
        if transaction < 0:
            ...
            for month in no_monthly_transactions:
                no_monthly_transactions[month] = no_monthly_transactions.get(month, 0) + 1
            #create a dictionary containing the value of card payments per month
            for month in value_monthly_transactons:
                value_monthly_transactons[month] = value_monthly_transactons.get(month, 0) + transaction
